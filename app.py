...
import pandas as pd
from algo import fcfs,sstf,scan,cscan
...
max_cylinder = st.sidebar.number_input("Maximum Cylinder/Track Number", min_value=1, value=200)

algorithm = st.sidebar.selectbox("Algorithm", ["FCFS","SSTF", "SCAN", "C-SCAN"])
...
if algorithm == "FCFS":
    ...
    sequence,distance  = sstf(requests, initial_head)
elif algorithm == "SCAN":
    sequence,distance  = scan(requests, initial_head,max_cylinder,direction)
elif algorithm == "C-SCAN":
    sequence,distance  = cscan(requests, initial_head,max_cylinder,direction)
